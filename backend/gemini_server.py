...
load_dotenv(os.path.join(os.path.dirname(__file__), '.env'))  # Load .env file from same directory
...
def get_coaching_tip():
    ...
    try:
        ...
        error_msg = str(e)
        print(f"Error generating coaching tip: {error_msg}")
        
        # Determine if it's a quota issue (429)
        is_quota_error = "429" in error_msg
        
        fallback_tip = "You're doing great! Focus on balanced meals and staying hydrated."
        if is_quota_error:
            fallback_tip = "I'm a bit busy right now (Quota Exceeded), but keep up the good work! Try a lighter meal next."
            
        return jsonify({
            "error": error_msg,
            "status": "fallback", 
            "coaching_tip": fallback_tip
        }), 200
